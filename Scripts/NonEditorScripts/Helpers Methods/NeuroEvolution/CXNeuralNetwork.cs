...
#pragma warning restore IDE0017
        ...
        /// <summary>
        /// Fully mutates (Randomize the whole value) the Neural Network
        /// </summary>
        public void FullMutate(float mutateRate = .2f) =>
            MapWeights((i, x, y) => MathUtils.FlipCoin(Random.Range(-1f, 1f), weights[i][x, y], mutateRate));

        /// <summary>
        /// Nudges the value by the nudge rate with the whole neural network
        /// </summary>
        public void NudgeMutate(float nudgeRate = .1f, float mutateRate = .2f) =>
            MapWeights((i, x, y) =>
           MathUtils.FlipCoin(
               Mathf.Clamp(weights[i][x, y] + Random.Range(-nudgeRate, nudgeRate), -1f, 1f),
               weights[i][x, y],
               mutateRate)
            );
        ...
        {
            CXNeuralNetwork child = new CXNeuralNetwork(InputCount, HiddenLayerCount, HiddenNeuronCount, OutputCount);

            child.MapWeights((i, x, y) =>
                MathUtils.FlipCoin(rate) ? weights[i][x, y] : other.weights[i][x, y]);
