...
namespace CXUtils.CodeUtils
{
    ...
    public class RandomUtils : IBaseUtils
    {
        ...
        public T FlipCoin<T>(params T[] items) =>
            items[Random.Range(0, items.Length - 1)]; //in here, the random that comes from unity will return the upper bound, so -1
    }
    ...
    public class CXRandom : SysRandom
    {
        ...
        public bool FlipCoin(double threashold = .5f) =>
            NextDouble() > threashold;
        ...
        public T FlipCoin<T>(params T[] items) =>
            items[Next(0, items.Length)]; //in here, the random value will never return the upper bound, so don't worry
